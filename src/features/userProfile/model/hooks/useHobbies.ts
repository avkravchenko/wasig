...
const useHobbies = (setVisible: (visible: boolean) => void, onNextStep: () => void) => {
    ...
    const [customHobbyToDisplay, setCustomHobbyToDisplay] = useState<CustomHobby[]>([]);
    const [selectedCustomHobbies, setSelectedCustomHobbies] = useState<Set<string>>(new Set());
    ...
    const isCustomHobbyUnique = !trimmedInput || 
      !customHobbyToDisplay.some(item => item.name.toLowerCase() === trimmedInput);
